...
// solo, con setTimeout, en loop). Después el usuario pidió que este bloque
// fuera un acordeón: colapsado por defecto, con un botón "ver pipeline de
// producción" (ícono + chevron) para desplegarlo — este test primero
// confirma que arranca colapsado y que el botón lo despliega, y luego que
// las 4 etapas están en el orden esperado y que el loop realmente avanza
// solo hasta completar "deploy" en verde, sin que haya que hacer nada más.
...
const hiddenBefore = await page.locator('#chzPipeline').evaluate(el => el.hidden);
assert(hiddenBefore === true, 'el pipeline debería arrancar colapsado (acordeón cerrado por defecto)');

await page.click('#chzPipelineToggle');
await page.waitForTimeout(150);
const hiddenAfter = await page.locator('#chzPipeline').evaluate(el => el.hidden);
assert(hiddenAfter === false, 'apretar el botón "ver pipeline de producción" debería desplegar el bloque');
...
await close();
console.log('  Pipeline de la Malla OK — arranca colapsado, el botón "ver pipeline de producción" lo despliega, las 4 etapas (build/lint/test/deploy) existen y el loop avanza solo hasta completar en verde');
